  ...
  <div class="divBox">
      ...
      <el-form :model="query" :inline="true" label-width="80px">
        ...
        <el-form-item prop="status" label="状态">
          <el-select v-model="query.status">
            <el-option :key="item.value" v-for="item in statusList" :value="item.value" :label="item.label" clearable></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-card class="mt12">
      ...
      <el-table :data="tableData" highlight-current-row header-cell-class-name="table-header">
        ...
        <el-table-column prop="alias" label="字典别名">
          <template v-slot="scope">
            <el-button
              type="text"
              size="small"
              @click="goToDetail(scope.row)"
            >{{scope.row.alias}}</el-button>
          </template>
...
import {getDictTypeList, dictTypeStore, dictTypeDel, dictTypeInfo} from "@/api/tool/dict_type";
