...
<template>
  <div class="base-table">
    ...
    <el-table :ref="dropRef" :max-height="maxHeight" :data="list" border style="width: 100%" :size="tableSize" @selection-change="handleSelectionChange">
      <el-table-column
        v-if="showCheckbox"
        type="selection"
        width="55">
...
export default {
  ...
  computed: {
    ...
    customWidth() {
      return this.tableHead.length < 7 ? null : 200
    }
  },
  watch: {
    tableData: {
    ...
    handleSelectionChange(selectIds) {
      if (!this.isSingle) {
        ...
        if (selectIds.length > 1) {
          this.$refs[this.dropRef].clearSelection();
          this.$refs[this.dropRef].toggleRowSelection(selectIds.pop());
          this.$emit('handleSelectionChange', [selectIds.pop()])
        } else {
          this.$emit('handleSelectionChange', selectIds)
        }
      }
